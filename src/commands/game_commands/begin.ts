...
export default class BeginCommand implements BaseCommand {
    async call({ message, gameSessions }: CommandArgs) {
        ...
        if (gameSessions[guildID].owner.id !== author.id) {
            return;
        }
        const guildPreference = await getGuildPreference(guildID);
        if (gameSessions[guildID] && gameSessions[guildID].eliminationMode === GameType.ELIMINATION && !gameSessions[guildID].sessionInitialized) {
