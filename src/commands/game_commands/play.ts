...
import { deleteGameSession, getRestartTime } from "../../helpers/management_utils";
...
export default class PlayCommand implements BaseCommand {
    ...
    async call({ message, gameSessions, parsedMessage, channel }: CommandArgs) {
        ...
        const restartImpending = await getRestartTime();
        if (restartImpending) {
            sendErrorMessage(MessageContext.fromMessage(message), { title: "Cannot start new game", description: "Bot is restarting soon, please wait until the bot is back up!" });
            return;
        }
