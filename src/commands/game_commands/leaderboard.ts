...
export default class LeaderboardCommand implements BaseCommand {
        ...
        examples: [
            {
                ...
                explanation: state.localizer.translate(
                    guildID,
                    "leaderboard.help.example.global"
                ),
            },
            {
    ...
    call = async ({ message, parsedMessage }: CommandArgs): Promise<void> => {
        ...
        if (pageOffset === 0 && !type && !scope && !duration) {
            sendValidationErrorMessage(
                ...
                state.localizer.translate(
                    message.guildID,
                    "leaderboard.validation.firstArg",
                    {
        ...
        if (Object.values(LeaderboardScope).includes(arg as LeaderboardScope)) {
            ...
            sendValidationErrorMessage(
                ...
                state.localizer.translate(
                    message.guildID,
                    "leaderboard.validation.secondArg",
                    {
        ...
        if (pageOffset === 0 && parsedMessage.components.length > 3) {
            sendValidationErrorMessage(
                ...
                state.localizer.translate(
                    message.guildID,
                    "leaderboard.validation.thirdArg"
                ),
                arg,
                this.help(message.guildID).usage
            );
                        ...
                        switch (type) {
                            ...
                            case LeaderboardType.GAMES_PLAYED:
                                leaderboardType = state.localizer.translate(
                                    messageContext.guildID,
                                    "leaderboard.byGamesPlayed"
                                );
                                break;
                            case LeaderboardType.SONGS_GUESSED:
                                leaderboardType = state.localizer.translate(
                                    messageContext.guildID,
                                    "leaderboard.bySongsGuessed"
                                );
                                break;
                            default:
                                break;
    ...
    private static async enrollLeaderboard(
        ...
        if (alreadyEnrolled) {
            sendErrorMessage(MessageContext.fromMessage(message), {
                title: state.localizer.translate(
                    message.guildID,
                    "leaderboard.failure.alreadyEnrolled.title"
                ),
                description: state.localizer.translate(
                    message.guildID,
        ...
        sendInfoMessage(MessageContext.fromMessage(message), {
            title: state.localizer.translate(
                message.guildID,
                "leaderboard.enrolled.title"
            ),
            description: state.localizer.translate(
                message.guildID,
                "leaderboard.enrolled.description"
            ),
        });
    }
    ...
    private static async unenrollLeaderboard(
        ...
        sendInfoMessage(MessageContext.fromMessage(message), {
            title: state.localizer.translate(
                message.guildID,
                "leaderboard.unenrolled.title"
            ),
            description: state.localizer.translate(
                message.guildID,
                "leaderboard.unenrolled.description"
            ),
        });
    }
    ...
    private static async showLeaderboard(
        ...
        if (scope === LeaderboardScope.GAME) {
            if (!state.gameSessions[message.guildID]) {
                sendErrorMessage(messageContext, {
                    ...
                    description: state.localizer.translate(
                        message.guildID,
                        "leaderboard.failure.game.noneInProgress.description"
                    ),
            ...
            if (participantIDs.size === 0) {
                sendErrorMessage(messageContext, {
                    title: state.localizer.translate(
                        message.guildID,
                        "leaderboard.failure.game.noParticipants.title"
                    ),
                    description: state.localizer.translate(
                        message.guildID,
        ...
        if (pageCount === 0) {
            sendErrorMessage(messageContext, {
                title: state.localizer.translate(
                    message.guildID,
                    "leaderboard.failure.empty.title"
                ),
                description: state.localizer.translate(
                    message.guildID,
        ...
        if (pageOffset > pageCount) {
            sendErrorMessage(messageContext, {
                title: state.localizer.translate(
                    message.guildID,
                    "leaderboard.failure.outOfRange.title"
                ),
                description: state.localizer.translate(
                    message.guildID,
                    "leaderboard.failure.outOfRange.description"
                ),
                thumbnailUrl: KmqImages.NOT_IMPRESSED,
            });
            return;
