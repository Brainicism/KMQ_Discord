...
export default class ExpCommand implements BaseCommand {
    ...
    static async sendExpMessage(
        ...
        const totalModifier = await ExpCommand.calculateOptionsExpMultiplier(
            guildPreference,
            voteBonusActive,
            messageContext.author.id,
        );
     ...
     */
    static async calculateOptionsExpMultiplierInternal(
        ...
        if (
            guildPreference.gameOptions.shuffleType === ShuffleType.POPULARITY
        ) {
            modifiers.push({
                displayName: i18n.translate(
                    guildPreference.guildID,
            ...
            ].includes(guildPreference.gameOptions.shuffleType)
        ) {
            modifiers.push({
                displayName: i18n.translate(
                    guildPreference.guildID,
    ...
    static async calculateOptionsExpMultiplier(
        guildPreference: GuildPreference,
        voteBonusExp: boolean,
        playerID: string,
    ): Promise<number> {
     ...
     */
    static async calculateTotalRoundExp(
        guildPreference: GuildPreference,
        gameRound: GameRound,
        ...
        const optionsMultiplier =
            await ExpCommand.calculateOptionsExpMultiplier(
                guildPreference,
                voteBonusExp,
                playerID,
            );

        const roundMultipler = ExpCommand.calculateRoundExpMultiplier(
            gameRound,
            numParticipants,
            streak,
            guessSpeedMs,
     ...
     */
    static calculateRoundExpMultiplier(
        gameRound: GameRound,
        numParticipants: number,
        streak: number,
        guessSpeedMs: number,
     ...
     */
    static participantExpScalingModifier(numParticipants: number): number {
        return (
            1 +
            0.1 *
                (Math.min(
