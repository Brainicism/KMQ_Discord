...
export default class GoalCommand implements BaseCommand {
    ...
    async call({ message, parsedMessage, gameSessions }: CommandArgs) {
        ...
        if (parsedMessage.components.length === 0) {
            ...
            await sendOptionsMessage(message, guildPreference, { option: GameOption.GOAL, reset: true });
        ...
        if (gameSession && !gameSession.scoreboard.isEmpty() && userGoal <= gameSession.scoreboard.getWinners()[0].getScore()) {
            ...
            sendErrorMessage(getMessageContext(message), { title: "Error applying goal", description: "Given goal exceeds highest score. Please raise your goal, or start a new game." });
            return;
        }

        guildPreference.setGoal(userGoal);
        await sendOptionsMessage(message, guildPreference, { option: GameOption.GOAL, reset: false });
