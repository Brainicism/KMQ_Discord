...
async function generatePresetUUIDs() {
    ...
    if (presetsWithoutUUID.length === 0) {
        logger.info(`All ${(await presetsWithUUID).length} presets have UUIDs.`);
        return;
    }

    logger.info(`${(await presetsWithUUID).length} presets with UUID. Generating presets for ${presetsWithoutUUID.length} presets.`);
