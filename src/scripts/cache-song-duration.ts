...
async function cacheSongDuration(db: DatabaseContext): Promise<void> {
    ...
    for (const fileName of files) {
        ...
        if (!(await db.kmq("cached_song_duration").select("*").where("vlink", "=", vlink).first())) {
            // uncached song
            const songDuration = await getAudioDurationInSeconds(path.join(process.env.SONG_DOWNLOAD_DIR, fileName));
            ...
            if (cachedSongs % 100 === 0) {
                logger.info(`${cachedSongs} song durations were cached so far.`);
            }
        }
    }
