...
import { calculateOptionsExpMultiplierInternal, calculateRoundExpMultiplier, ExpBonusModifier, ExpBonusModifierValues, participantExpScalingModifier } from "../../commands/game_commands/exp";
...
describe("calculateOptionsMultiplier", () => {
    ...
    describe("no time-based modifiers", () => {
        ...
        describe("insufficient song count", () => {
            ...
            it("should return insufficient song count penalty", async () => {
                const modifiers = await calculateOptionsExpMultiplierInternal(guildPreference, false);
                assert.strictEqual(modifiers.length, 1);
                assert.strictEqual(modifiers[0].name, ExpBonusModifier.BELOW_SONG_COUNT_THRESHOLD);
            });
        });
        ...
        describe("sufficient song count", () => {
            ...
            describe("vote bonus", () => {
                it("should return vote bonus modifier", async () => {
                    ...
                    assert.strictEqual(modifiers.length, 1);
                    assert.strictEqual(modifiers[0].name, ExpBonusModifier.VOTE);
                });
            });

            describe("multiple choice penalty", () => {
                const multipleChoicePenaltyMap = {
                    [AnswerType.MULTIPLE_CHOICE_EASY]: ExpBonusModifier.MC_GUESS_EASY,
                    [AnswerType.MULTIPLE_CHOICE_MED]: ExpBonusModifier.MC_GUESS_MEDIUM,
                    [AnswerType.MULTIPLE_CHOICE_HARD]: ExpBonusModifier.MC_GUESS_HARD,
                };

                for (const answerType of Object.keys(multipleChoicePenaltyMap)) {
                    ...
                    it(`should return corresponding multiple choice penalty (${answerType})`, async () => {
                        ...
                        const modifiers = await calculateOptionsExpMultiplierInternal(guildPreference, false);
                        assert.strictEqual(modifiers.length, 1);
                        assert.strictEqual(modifiers[0].name, multipleChoicePenaltyMap[answerType]);
                    });
                }
            });
            ...
            describe("artist/group guess mode", () => {
                describe("artist guess mode", () => {
                    it("should return artist guess penalty", async () => {
                        await guildPreference.setGuessModeType(GuessModeType.ARTIST);
                        const modifiers = await calculateOptionsExpMultiplierInternal(guildPreference, false);
                        assert.strictEqual(modifiers.length, 1);
                        assert.strictEqual(modifiers[0].name, ExpBonusModifier.ARTIST_GUESS);
                    });
                });
                ...
                describe("groups mode also selected", () => {
                    it("should return artist guess + groups mode penalty", async () => {
                        await guildPreference.setGuessModeType(GuessModeType.BOTH);
                        await guildPreference.setGroups([{ id: 1, name: "aespa" }]);
                        const modifiers = await calculateOptionsExpMultiplierInternal(guildPreference, false);
                        assert.strictEqual(modifiers.length, 1);
                        assert.strictEqual(modifiers[0].name, ExpBonusModifier.ARTIST_GUESS_GROUPS_SELECTED);
                    });
                });
            });
        });
    ...
    describe("powerhour", () => {
        ...
        describe("is weekend", () => {
            it("should return power hour modifier", async () => {
                ...
                const modifiers = await calculateOptionsExpMultiplierInternal(guildPreference, false);
                assert.strictEqual(modifiers.length, 1);
                assert.strictEqual(modifiers[0].name, ExpBonusModifier.POWER_HOUR);
            });
        });
    ...
    describe("everything", () => {
        it("should return all bonuses/penalties", async () => {
            ...
            assert.ok(modifiers.map((x) => x.name).every((x) => [
...
describe("calculateRoundExpMultiplier", () => {
    ...
    describe("participant exp scaling", () => {
        it("should increase EXP until a limit", () => {
            let roundExp = calculateRoundExpMultiplier(gameRound, 1, 0, 1000, 1);
            for (let i = 2; i <= exp.PARTICIPANT_MODIFIER_MAX_PARTICIPANTS; i++) {
                const newRoundExp = calculateRoundExpMultiplier(gameRound, i, 0, 1000, 1);
                assert.ok(newRoundExp > roundExp);
                roundExp = newRoundExp;
            }

            const finalRoundExp = calculateRoundExpMultiplier(gameRound, exp.PARTICIPANT_MODIFIER_MAX_PARTICIPANTS + 1, 0, 1000, 1);
    ...
    describe("fast guess", () => {
        it("should apply fast guess bonus", () => {
            const slowGuessExp = calculateRoundExpMultiplier(gameRound, 1, 0, 10000, 1);
            const fastGuessExp = calculateRoundExpMultiplier(gameRound, 1, 0, 50, 1);
            assert.ok(fastGuessExp === slowGuessExp * ExpBonusModifierValues[ExpBonusModifier.QUICK_GUESS]);
        });
    });
    ...
    describe("guess streak", () => {
        it("should apply guess streak bonus", () => {
            ...
            for (let i = 2; i < exp.GUESS_STREAK_THRESHOLD; i++) {
                const newRoundExp = calculateRoundExpMultiplier(gameRound, 1, i, 1000, 1);
                assert.ok(newRoundExp === roundExp);
                roundExp = newRoundExp;
            }

            const finalRoundExp = calculateRoundExpMultiplier(gameRound, 1, exp.GUESS_STREAK_THRESHOLD, 1000, 1);
            assert.ok(finalRoundExp === roundExp * ExpBonusModifierValues[ExpBonusModifier.GUESS_STREAK]);
        });
    });

    describe("bonus artist", () => {
        it("should apply bonus artist bonus", () => {
            const regularExp = calculateRoundExpMultiplier(gameRound, 1, 0, 1000, 1);
            sandbox.stub(gameRound, "isBonusArtist").callsFake(() => true);
            const bonusArtistExp = calculateRoundExpMultiplier(gameRound, 1, 0, 1000, 1);
            assert.ok(bonusArtistExp === regularExp * ExpBonusModifierValues[ExpBonusModifier.BONUS_ARTIST]);
        });
    });
    ...
    describe("guess placement", () => {
        it("should apply guess placement penalty", () => {
            const place = 7;
            const regularExp = calculateRoundExpMultiplier(gameRound, 1, 0, 1000, 1);
            const lowerPlacementExp = calculateRoundExpMultiplier(gameRound, 1, 0, 1000, place);
    ...
    describe("everything", () => {
        it("should apply all modifiers", () => {
            const roundBonus = 50;
            const numParticipants = exp.PARTICIPANT_MODIFIER_MAX_PARTICIPANTS - 1;
            const place = 7;
            const guessStreak = exp.GUESS_STREAK_THRESHOLD + 1;
            gameRound.bonusModifier = roundBonus;
            sandbox.stub(gameRound, "isBonusArtist").callsFake(() => true);
            const expModifier = calculateRoundExpMultiplier(gameRound, numParticipants, guessStreak, 50, place);
            assert.strictEqual(expModifier,
