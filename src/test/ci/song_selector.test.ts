        ...
        describe("single-select gender", () => {
            it("should match the expected song count", async () => {
                for (const gender of [Gender.MALE, Gender.FEMALE, Gender.COED]) {
                    ...
                    const { songs } = await SongSelector.getFilteredSongList(guildPreference);
    ...
    describe("gender override", () => {
        ...
        describe("override to female", () => {
            it("should match the expected song count of female + coed songs", async () => {
                ...
                const filteredSongs = (await SongSelector.getFilteredSongList(guildPreference)).songs;
                ...
                for (let i = 0; i < femaleOrCoedSongCount; i++) {
                    femaleOrCoedSongs.push(await SongSelector.selectRandomSong(filteredSongs, new Set(femaleOrCoedSongs.map((x) => x.youtubeLink)), Gender.FEMALE));
                }
        ...
        describe("override to male", () => {
            it("should match the expected song count of male + coed songs", async () => {
                ...
                for (let i = 0; i < maleOrCoedSongCount; i++) {
                    maleOrCoedSongs.push(await SongSelector.selectRandomSong(filteredSongs, new Set(maleOrCoedSongs.map((x) => x.youtubeLink)), Gender.MALE));
                }
        ...
        describe("ignoring first 10 songs", () => {
            it("should match the expected song count", async () => {
                ...
                for (let i = 0; i < filteredSongs.size - numIgnored; i++) {
                    selectedSongs.push(await SongSelector.selectRandomSong(filteredSongs, new Set([...ignoredSongs, ...selectedSongs])));
                }
        ...
        describe("selected songs changes midway", () => {
            ...
            describe("new selected song set is a superset of the original, new selected song set has already been played", () => {
                it("should reset the unique song queue", async () => {
                    ...
                    await guildPreference.setLimit(0, numberSongs);
                    await songSelector.reloadSongs(guildPreference);
                    let songs = [...songSelector.getSongs().songs].map((x) => x.youtubeLink);
            ...
            describe("unique song history has songs not in the current selected song set", () => {
                it("should reset the unique song queue", async () => {
                    ...
                    songSelector.uniqueSongsPlayed = new Set(songs.slice(0, numberSongs - songsNotInSet.length).concat(songsNotInSet));
                    assert.strictEqual(songSelector.checkUniqueSongQueue(guildPreference), false);
