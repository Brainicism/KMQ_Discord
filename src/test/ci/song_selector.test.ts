...
import { mockArtists, mockSongs, MOCK_SONG_COUNT } from "../test_setup";
import SongSelector, {
    ...
    describe("premium", () => {
        describe("non-premium songs", () => {
            it("should match the expected song count", async () => {
                ...
                const nonPremiumSongs = [...songs].filter(
                    (x) => x.rank <= Number(process.env.AUDIO_SONGS_PER_ARTIST)
                );

                assert.strictEqual(songCount.count, MOCK_SONG_COUNT / 2);
