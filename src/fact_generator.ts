...
async function recentMusicVideos(lng: LocaleType): Promise<string[]> {
    ...
    return result.map((x) =>
        LocalizationManager.internalLocalizer.t("fact.fun.newMV", {
...
async function recentMilestone(lng: LocaleType): Promise<string[]> {
    ...
    return result.map((x) =>
        LocalizationManager.internalLocalizer.t("fact.fun.mvViewMilestone", {
            hyperlink: generateSongArtistHyperlink(
                lng,
                x["song_name"],
                x["artist_name"],
...
async function recentMusicShowWin(lng: LocaleType): Promise<string[]> {
    ...
    return result.map((x) =>
        LocalizationManager.internalLocalizer.t("fact.fun.recentMusicShowWin", {
            hyperlink: generateSongArtistHyperlink(
                lng,
                x["winning_song"],
                x["artist_name"],
...
async function musicShowWins(lng: LocaleType): Promise<string[]> {
    ...
    return result.map((x, idx) =>
        LocalizationManager.internalLocalizer.t("fact.fun.mostMusicShowWins", {
            artist: x["artist_name"],
            ordinalNum: LocalizationManager.internalLocalizer.t(
                getOrdinalNum(idx + 1),
                { lng }
            ),
            num: x["count"],
...
async function mostViewedGroups(lng: LocaleType): Promise<string[]> {
    ...
    return result.map((x, idx) =>
        LocalizationManager.internalLocalizer.t("fact.fun.mostViewedGroup", {
            artist: x["artist_name"],
            ordinalNum: LocalizationManager.internalLocalizer.t(
                getOrdinalNum(idx + 1),
                { lng }
...
async function mostLikedGroups(lng: LocaleType): Promise<string[]> {
    ...
    return result.map((x, idx) =>
        LocalizationManager.internalLocalizer.t("fact.fun.mostLikedGroup", {
            artist: x["artist_name"],
            ordinalNum: LocalizationManager.internalLocalizer.t(
                getOrdinalNum(idx + 1),
                { lng }
...
async function mostViewedVideo(lng: LocaleType): Promise<string[]> {
    ...
    return result.map((x, idx) =>
        LocalizationManager.internalLocalizer.t("fact.fun.mostViewedVideo", {
            hyperlink: generateSongArtistHyperlink(
                lng,
                x["song_name"],
                x["artist_name"],
...
async function mostLikedVideo(lng: LocaleType): Promise<string[]> {
    ...
    return result.map((x, idx) =>
        LocalizationManager.internalLocalizer.t("fact.fun.mostLikedVideo", {
            hyperlink: generateSongArtistHyperlink(
                lng,
                x["song_name"],
                x["artist_name"],
...
async function mostMusicVideos(lng: LocaleType): Promise<string[]> {
    ...
    return result.map((x, idx) =>
        LocalizationManager.internalLocalizer.t("fact.fun.mostMVs", {
            artist: x["artist_name"],
            ordinalNum: LocalizationManager.internalLocalizer.t(
                getOrdinalNum(idx + 1),
                { lng }
...
async function yearWithMostDebuts(lng: LocaleType): Promise<string[]> {
    ...
    return result.map((x, idx) =>
        LocalizationManager.internalLocalizer.t("fact.fun.mostDebuts", {
...
async function yearWithMostReleases(lng: LocaleType): Promise<string[]> {
    ...
    return result.map((x, idx) =>
        LocalizationManager.internalLocalizer.t("fact.fun.mostActiveYear", {
...
async function viewsByGender(lng: LocaleType): Promise<string[]> {
    ...
    return [
        LocalizationManager.internalLocalizer.t("fact.fun.viewsByGender", {
...
async function mostViewedSoloArtist(lng: LocaleType): Promise<string[]> {
    ...
    return result.map((x, idx) =>
        LocalizationManager.internalLocalizer.t("fact.fun.mostViewedSoloist", {
            artist: x["artist_name"],
            ordinalNum: LocalizationManager.internalLocalizer.t(
                getOrdinalNum(idx + 1),
                { lng }
...
async function viewsBySolo(lng: LocaleType): Promise<string[]> {
    ...
    return [
        LocalizationManager.internalLocalizer.t("fact.fun.viewsByArtistType", {
            totalViews: friendlyFormattedNumber(totalViews),
...
async function songReleaseAnniversaries(lng: LocaleType): Promise<string[]> {
    ...
    return result.map((x) =>
        LocalizationManager.internalLocalizer.t("fact.fun.oldMV", {
            hyperlink: generateSongArtistHyperlink(
                lng,
                x["song_name_en"],
                x["artist_name_en"],
...
async function songGuessRate(lng: LocaleType): Promise<string[]> {
    ...
    return result.map((x) =>
        LocalizationManager.internalLocalizer.t("fact.kmq.guessRate", {
            hyperlink: generateSongArtistHyperlink(
                lng,
                x["song_name_en"],
                x["artist_name_en"],
...
async function bigThreeDominance(lng: LocaleType): Promise<string[]> {
    ...
    return [
        LocalizationManager.internalLocalizer.t("fact.fun.bigThreeDominance", {
...
async function fanclubName(lng: LocaleType): Promise<Array<string>> {
    ...
    return result.map((x) =>
        LocalizationManager.internalLocalizer.t("fact.fun.fanclubName", {
            name: x["name"],
            fanclub: x["fanclub"],
            lng,
        })
...
async function closeBirthdays(lng: LocaleType): Promise<Array<string>> {
    ...
    return result.map((x) =>
        LocalizationManager.internalLocalizer.t("fact.fun.birthday", {
            name: x["name"],
...
async function longestGame(lng: LocaleType): Promise<string[]> {
    ...
    return [
        LocalizationManager.internalLocalizer.t("fact.kmq.longestGame", {
...
async function mostGames(lng: LocaleType): Promise<string[]> {
    ...
    return [
        LocalizationManager.internalLocalizer.t("fact.kmq.mostActiveServer", {
...
async function globalTotalGames(lng: LocaleType): Promise<string[]> {
    ...
    return [
        LocalizationManager.internalLocalizer.t("fact.kmq.totalGames", {
...
async function recentGameSessions(lng: LocaleType): Promise<string[]> {
    ...
    return [
        LocalizationManager.internalLocalizer.t("fact.kmq.recentSessions", {
...
async function recentGames(lng: LocaleType): Promise<string[]> {
    ...
    return [
        LocalizationManager.internalLocalizer.t("fact.kmq.recentGameCount", {
...
async function recentUniquePlayers(lng: LocaleType): Promise<string[]> {
    ...
    return Promise.all(
        intervals.map(async (interval): Promise<string> => {
            ...
            return LocalizationManager.internalLocalizer.t(
                "fact.kmq.uniquePlayers",
                {
                    recentActivePlayers:
                        friendlyFormattedNumber(recentActivePlayers),
                    xDays: LocalizationManager.internalLocalizer.t(
                        "misc.plural.day",
                        {
                            count: interval,
                            lng,
...
async function mostGaonFirsts(lng: LocaleType): Promise<string[]> {
    ...
    return result.map((x, idx) =>
        LocalizationManager.internalLocalizer.t("fact.fun.mostGaonFirsts", {
...
async function recentGaonWeekly(lng: LocaleType): Promise<Array<string>> {
    ...
    return parsedResult.slice(0, 10).map((x, idx) =>
        LocalizationManager.internalLocalizer.t("fact.fun.recentGaonWeekly", {
            songName: generateSongArtistHyperlink(
                lng,
                x["songName"],
                x["artistName"]
