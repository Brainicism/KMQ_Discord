...
import { LanguageType, FOREIGN_LANGUAGE_TAGS } from "../commands/game_options/language";
import { OstPreference } from "../commands/game_options/ost";
import { ReleaseType, NON_OFFICIAL_VIDEO_TAGS } from "../commands/game_options/release";
...
export default class SongSelector {
    ...
    checkUniqueSongQueue(guildPreference: GuildPreference): boolean {
        ...
        if (guildPreference.gameOptions.shuffleType === ShuffleType.UNIQUE) {
            const filteredSongs = new Set([...selectedSongs].map((x) => x.youtubeLink));
    ...
    async queryRandomSong(guildPreference: GuildPreference): Promise<QueriedSong> {
        ...
        if (this.lastAlternatingGender) {
            randomSong = await SongSelector.selectRandomSong(selectedSongs, ignoredSongs, this.lastAlternatingGender);
        } else {
            randomSong = await SongSelector.selectRandomSong(selectedSongs, ignoredSongs);
        }
     ...
     */
    static async selectRandomSong(filteredSongs: Set<QueriedSong>, ignoredSongs?: Set<string>, alternatingGender?: Gender): Promise<QueriedSong> {
    ...
    async reloadSongs(guildPreference: GuildPreference): Promise<void> {
        this.filteredSongs = await SongSelector.getFilteredSongList(guildPreference);
    }
     ...
     */
    static async getFilteredSongList(guildPreference: GuildPreference): Promise<{ songs: Set<QueriedSong>, countBeforeLimit: number }> {
