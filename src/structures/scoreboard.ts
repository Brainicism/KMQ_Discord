...
export default class Scoreboard {
    ...
    getScoreboardEmbedFields(playerRoundResultIDs?: Set<string>): Array<{ name: string, value: string, inline: boolean }> {
        return Object.values(this.players)
            .sort((a, b) => b.getScore() - a.getScore())
            .map((x, index) => (
                {
                    name: `${index + 1}. ${x.getDisplayedName(playerRoundResultIDs?.has(x.getID()), true)}`,
     ...
     */
    getScoreboardEmbedThreeFields(cutoff: number, playerRoundResultIDs?: Set<string>): Array<{ name: string, value: string, inline: boolean }> {
        ...
        const players = Object.values(this.players)
            ...
            .map((x, index, arr) => {
                ...
                return `${bold(String(index + 1))}. ${x.getDisplayedName(playerRoundResultIDs?.has(x.getID()), duplicateName)}: ${x.getDisplayedScore()}`;
            });
