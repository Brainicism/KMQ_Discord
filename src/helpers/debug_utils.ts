...
export function isDebugMode(): boolean {
    let developmentBuild = process.env.NODE_ENV === "development";
...
export async function getForcePlaySong(db: Databases): Promise<QueriedSong> {
    if (!isDebugMode()) return null;
    let forcePlaySongId = readDebugSettings("forcedSongId");
...
function readDebugSettings(key: string): any {
    let debugSettings = JSON.parse(fs.readFileSync(path.resolve(__dirname, "../../debug_settings.json")).toString());
