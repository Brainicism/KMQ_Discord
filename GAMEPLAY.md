...
## ,limit [x]
Setting a limit 'limits' KMQ bot to the top most viewed `x` music videos out of the total number of songs (the number underlined in `,options`).
...
Setting the groups option limits the selection of songs to those belonging to the artist specified. For instance `,groups blackpink,itzy,bts` would exclusively play songs from those three artists. In order to reset this option, simply type `,groups`. You can view the list of groups with `,help groups`. Make sure to separate the groups with commas.
