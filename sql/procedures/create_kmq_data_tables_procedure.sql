...
BEGIN
	...
	CREATE TABLE available_songs_temp (
		...
		alphanumeric_song_name_en VARCHAR(255) NOT NULL,
		...
		alphanumeric_artist_name_en VARCHAR(255) NOT NULL,
	...
	SELECT
		...
		REGEXP_REPLACE(LOWER(TRIM(SUBSTRING_INDEX(kpop_videos.app_kpop.name, '(', 1))), '[^0-9a-z]', '') AS alphanumeric_song_name_en,
		...
		REGEXP_REPLACE(LOWER(TRIM(kpop_videos.app_kpop_group.name)), '[^0-9a-z]', '') AS alphanumeric_artist_name_en,
	...
	FROM (
		SELECT
			...
			REGEXP_REPLACE(LOWER(TRIM(SUBSTRING_INDEX(kpop_videos.app_kpop_audio.name, '(', 1))), '[^0-9a-z]', '') AS alphanumeric_song_name_en,
